        ...
        <form method="POST" action="{{ route('product.update' , $product->id) }}" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="pd-20 card-box mb-30">
                        ...
                        <select name="industry_id" id="industry_id" class="custom-select2 form-control @error('industry_id') is-invalid @enderror" style="width: 350px !important;">
                            ...
                            <optgroup label="Industry Name">
                                @foreach($industriesName as $value)
                                    <option value="{{ $value->id }}" {{ $product->industry_id == $value->id ? 'selected' : '' }}>{{ $value->industries_name }}</option>
                                @endforeach
                            </optgroup>
                        </select>
                        @error('industry_id')
                                ...
                                @foreach($industriesCategoryName as $categories)
                                    @foreach($categories as $category)
                                        <option value="{{ $category }}"
                                            {{ $product->categoryName === $category ? 'selected' : '' }}>
                                            {{ $category }}
                                        </option>
                                    @endforeach
                                @endforeach
                ...
                <div class="form-group row mt-3">
                            ...
                            <optgroup label="Filter Name">
                                @foreach ($filterNames as $filter)
                                    @foreach ($filter as $name)
                                        <option value="{{ $name }}" {{ $name == $product->filterName ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                @endforeach
                    ...
                    <div class="col-sm-4 col-md-4">
                        <input type="text" name="productName" id="productName" class="form-control @error('productName') is-invalid @enderror" placeholder="Enter Product Name" value="{{ $product->productName }}">
                        @error('productName')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <tbody>
                        ...
                        @else
                            <tr>
                                <td>
                                    <div class="col-sm-12 col-md-12">
                                        <input type="text" name="productCategoryName[]" class="form-control @error('productCategoryName.*') is-invalid @enderror" value="{{ old('productCategoryName.0') }}" placeholder="Enter Product Category Name">
                                        @error('productCategoryName.*')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
